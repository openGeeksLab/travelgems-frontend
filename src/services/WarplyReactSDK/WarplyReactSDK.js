...
import configureSDKStore from './redux/stores/configureSDKStore';
import DeviceInfo from './micro_apps/DeviceInfo';


export const store = configureSDKStore();

export class WarplyReactSDK {
  /*
...
//    var microapps = store.getState().reducers.ContextVariables.microapps ? store.getState().reducers.ContextVariables.microapps : default_mapps;
//    var mapp = null;
...
//    }
  ...
  static handleRegister(response){
    store.dispatch(actions.setWebId(response.data.context.web_id));
    store.dispatch(actions.setApiKey(response.data.context.api_key));
    this.getContext();
  };

  static handleGetContext(response){
    store.dispatch(actions.setContextVariables(response.data.context));
    this.initMicroApps();
  }
  ...
  static isRegistered(performRegister=true){
    var registered = store.getState().reducers.WebId && store.getState().reducers.ApiKey;
