...
export default function Day(props) {
  ...
  if (
    ...
    if (Array.isArray(minRangeDuration)) {
      let i = minRangeDuration.findIndex(
        (i) => i.date === selectedStartDate.valueOf(),
      );
      if (
        i >= 0 &&
        selectedStartDate.valueOf() +
          minRangeDuration[i].minDuration * 86400000 >
          thisDay.valueOf()
      ) {
  ...
  if (
    ...
    if (Array.isArray(maxRangeDuration)) {
      let i = maxRangeDuration.findIndex(
        (i) => i.date === selectedStartDate.valueOf(),
      );
      if (
        i >= 0 &&
        selectedStartDate.valueOf() +
          maxRangeDuration[i].maxDuration * 86400000 <
          thisDay.valueOf()
      ) {
    ...
    if (allowRangeSelection) {
      ...
      if (
        ...
        daySelectedStyle = [
          styles.selectedDay,
          selectedRangeStyle,
          selectedRangeStartStyle || styles.selectedDayBackground,
        ];
        selectedDayColorStyle = styles.selectedDayLabel;
      }
    }
